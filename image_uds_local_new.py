...
def second_level_person(state: ImageTaggingState) -> ImageTaggingState:
    ...
    if "人像" not in main_labels:
        ...
        - 人数：单人、多人（画面里出现≥2个人物）
        - 构图：自拍（人物手持手机 / 露出手臂、有自拍杆、高角度俯拍、近距离拍摄，满足任一即为自拍）、合影（多人同框且分布均匀）、正面（人脸对称）、侧面（单侧脸颊 / 眼睛占比大）、全身（画面完整容纳人物头顶至脚底或脚尖，能同时看到头和脚即为全身）、半身（画面容纳人物头顶至大腿中部或腰部之间）、面部特写（画面仅保留人物头部或完整面部，主体占比≤30%，含不超过颈部 1/3 长度且占比符合也为面部特写）
