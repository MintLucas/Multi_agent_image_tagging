...
def third_level_person_cloth(state: ImageTaggingState) -> ImageTaggingState:
    ...
    if "人像" not in main_labels:
        ...
        # """
        prompt = """
        任务：基于图片，提取“人像”的服饰与造型标签。
        
        【扫描顺序】：
        1. **衣物可视分析**：首先明确回答，画面中是否能看到人物的躯干（肩膀以下）？如果是大头照（面部特写）或光线全黑，请明确写出“看不到衣服”。
        2. **是否存在有效衣物**：基于分析，如果看不到衣服，必须选 False。
        3. **提取标签**：当 True 时，请先观察人物全身/半身的着装风格，再观察头部与颈部配饰，最后观察面部细节，从以下选项选择。
        
        请从以下预设选项中选择（可多选，可为空，不确定的标签坚决不选）：
