...
def first_level_classification(state: ImageTaggingState) -> ImageTaggingState:
    ...
    all_response = model.call_qwen_local_vl0(image_info, prompt)
    # all_response = model.call_qwen_local_vl0(image_info, prompt)
...
def second_level_person(state: ImageTaggingState) -> ImageTaggingState:
    ...
    if "人像" not in main_labels:
        ...
        response = model.call_qwen_local_vl0(image_info, prompt)
...
with open("image_uds_local0.png", "wb") as f:
    f.write(png_data)
...
def process_single_image0(img_path: str) -> dict:
    """处理单张图片，返回指定格式的结果字典"""
    try:
